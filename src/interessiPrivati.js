...
var vuedata = {
  ...
  showAllCharts: true,
  legislature: 'XIX',
  availableLegislatures: ['XVIII', 'XIX'],
  peopleGovOrLeg: {
    'XIX': ['Legislatura XIX', 'Governo Meloni I'],
    'XVIII': ['Legislatura XVIII', 'Governo Conte I', 'Governo Conte II', 'Governo Draghi I']
  },
  chartMargin: 40,
  travelFilter: 'all',
  charts: {
  ...
  colors: {
    ...
    istituzioni: {
      ...
      "Ministero dell'economia e delle finanze": "#0d769c",
      "Ministero dell'istruzione e del merito": "#065a77",
      "Ministero della difesa": "#014962",
    },
    parties: {
...
var urlLegislature = getParameterByName('legislature');
if(urlLegislature && vuedata.availableLegislatures.indexOf(urlLegislature) > -1) {
  vuedata.legislature = urlLegislature;
}
...
csv('./data/interessi-privati_'+vuedata.legislature+'.csv?' + randomPar, (err, interessi) => {
  csv('./data/persone.csv?' + randomPar, (err, persone) => {
    csv('./data/regioni.csv?' + randomPar, (err, regioni) => {
      //Filter persone based on selected legislature
      persone = _.filter(persone, function (x) { return vuedata.peopleGovOrLeg[vuedata.legislature].indexOf(x.government_or_legislature.trim()) > -1 });
      ...
      _.each(interessi, function (d) {
        ...
        d.person.person_institution_streamlined = d.person.person_institution;
        if(d.person.person_institution_streamlined.includes('Ministero')) {
          d.person.person_institution_streamlined = 'Ministero';
        }
      ...
      var createIstituzioniChart = function() {
        ...
        var dimension = ndx.dimension(function (d) {
          if(vuedata.legislature == 'XIX') { return d.person.person_institution_streamlined }
        ...
        chart
          ...
          */
          .colors(d3.scaleOrdinal().range(["#3dbeeb", "#00a7e1", "#0e8fc0", "#0d769c", "#065a77", "#014962", "#073849", "#1d86ac"]))
          .group(customGroup)
